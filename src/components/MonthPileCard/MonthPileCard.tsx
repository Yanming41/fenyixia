...
export const MonthPileCard = forwardRef<HTMLDivElement, MonthPileCardProps>(
    ({ monthKey, bills, style, onClick }, ref) => {
        const total = bills.reduce((s, b) => s + (b.total_amount || 0), 0);
        const icons = [...new Set(bills.map(b => b.icon))].slice(0, 5);

        return (
            <div ref={ref} className={styles.pile} style={style} onClick={onClick}>
